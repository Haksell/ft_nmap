...
static void set_port_and_host_state(
...
void scan_connect(t_thread_info* th_info) {
    ...
    for (int i = 0; i < port_count && run; ++i) {
        uint16_t actual_index = nmap->opt & OPT_NO_RANDOMIZE ? i
                                                             : nmap->random_indices[i];
        ...
        if (connect(fd, (struct sockaddr*)&targets[i], sizeof(target)) == -1 &&
            errno != EINPROGRESS) {
            set_port_and_host_state(th_info, PORT_CLOSED, port);
        }
    }

    uint64_t full_timeout = th_info->latency == 0 ? 3000000
                                                  : 2 * th_info->latency + 1000000;
    uint64_t start_time = get_microseconds();

    while (run) {
        uint64_t us_since_start = get_microseconds() - start_time;
        uint64_t remaining_timeout = full_timeout - us_since_start;
        uint64_t timeout_us = us_since_start >= full_timeout ||
                                      remaining_timeout <= 200000
                                  ? 200000
                                  : remaining_timeout;
        ...
        for (int i = 0; i < port_count; ++i) {
            if (fds[i].revents & POLLOUT || fds[i].revents & POLLERR) {
                ...
                int ret = getsockopt(
                    ...
                    SO_ERROR,
                    &so_error,
                    &(socklen_t){sizeof(so_error)}
                );
                if (ret == -1) error("getsockopt failed");
                if (so_error == 0)
                    set_port_and_host_state(
                        th_info,
                        PORT_OPEN,
                        nmap->port_array[actual_index]
                    );
