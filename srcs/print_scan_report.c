...
static void copy_port_state_combination(
    t_thread_info* th_info,
    t_port_state combination[SCAN_MAX],
    int port_index
) {
...
static bool same_port_combination(
    t_thread_info* th_info,
    t_port_state combination[SCAN_MAX],
    int port_index
) {
    for (t_scan_type scan_type = 0; scan_type < SCAN_MAX; ++scan_type) {
        if (combination[scan_type] !=
...
static t_paddings compute_paddings(
    t_thread_info* th_info,
    int hide_count,
    ...
    bool may_have_two_columns = (nmap->scans & ~(1 << SCAN_UDP)) &&
                                (nmap->scans & (1 << SCAN_UDP));
    for (int port_index = 0; port_index < nmap->port_count; ++port_index) {
        ...
        for (t_scan_type scan_type = 0; scan_type < SCAN_MAX; ++scan_type) {
            ...
            paddings.port_states[scan_type] = MAX(
                paddings.port_states[scan_type],
                port_state_info[state].strlen
            );
        }
        if (hide_count == 0 || !same_port_combination(
                                   th_info,
            ...
            if (may_have_two_columns && !paddings.two_columns &&
                strcmp(tcp_service, udp_service) != 0)
...
static int find_most_common_port_state_combination(
    t_thread_info* th_info,
    t_port_state combination[SCAN_MAX]
) {
...
static void print_line(
    ...
    char* tcp_service = port == HIDE_LINE     ? ""
                        : port == HEADER_LINE ? "SERVICE"
                                              : nmap->tcp_services[port_index];
    char* udp_service = port == HIDE_LINE     ? ""
                        : port == HEADER_LINE ? "SERVICE"
                                              : nmap->udp_services[port_index];
    ...
    if (!paddings->two_columns)
        printf(
            " %-*s",
            has_udp ? paddings->udp_service : paddings->tcp_service,
            has_udp ? udp_service : tcp_service
        );
    ...
    if (paddings->two_columns)
        printf("  %-*s" SEPARATOR, paddings->tcp_service, tcp_service);
...
static void gotta_go_fast(
    ...
    for (int port_index = 0; port_index < th_info->nmap->port_count; ++port_index) {
        if (!hide_count || !same_port_combination(
                               th_info,
                               common_port_state_combination,
                               port_index
                           )) {
...
static void print_host_is_up(t_thread_info* th_info) {
    t_port_state common_port_state_combination[SCAN_MAX];
    int hide_count = find_most_common_port_state_combination(
        th_info,
        common_port_state_combination
    );
    t_paddings paddings = compute_paddings(
        th_info,
        hide_count,
        common_port_state_combination
    );
    ...
    if (!ip_to_hostname(th_info->hostaddr.sin_addr, host, sizeof(host)) ||
        strcmp(th_info->host->name, host) == 0)
        host[0] = '\0';
