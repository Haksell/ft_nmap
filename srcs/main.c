...
static void launch_threads(t_nmap* nmap) {
    for (nmap->started_threads = 0; nmap->started_threads < nmap->num_threads && run;
         ++nmap->started_threads) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < nmap.num_threads; ++i)
        pthread_join(nmap.threads[i].thread_id, NULL);
