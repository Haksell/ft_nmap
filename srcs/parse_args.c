...
static void parse_ports(char* value, t_nmap* nmap) {
    ...
    while (comma) {
        ...
        if (value == hyphen || hyphen == end - 1)
            atou_check(value, 0, UINT16_MAX, "port");
        ...
        if (hyphen) {
            *hyphen = '\0';
            ...
            if (left > right)
                panic_parsing(
                    "Your port range %d-%d is backwards. Did you mean "
                    "%d-%d?\nQUITTING!\n",
                    left,
                    right,
                    right,
...
static void parse_scan(char* value, uint16_t* scans) {
    ...
    while (comma) {
        ...
        if (value == comma || comma == end - 1)
            panic_parsing("nmap: invalid scan value (`%s')\n", comma);
        if (comma) *comma = '\0';

        bool valid_scan = false;
        for (t_scan_type scan_type = 0; scan_type < SCAN_MAX; ++scan_type) {
            if (!strcmp(value, scans_str[scan_type]) ||
                (value[0] == scans_str[scan_type][0] && value[1] == '\0')) {
                *scans |= 1 << scan_type;
                valid_scan = true;
                break;
            }
...
static void parse_file(char* filename, t_nmap* nmap) {
    ...
    if (!file)
        panic_parsing(
            "nmap: failed to open hosts file \"%s\": %s\n",
            filename,
            strerror(errno)
        );
    ...
    if (read_failed)
        panic_parsing(
            "nmap: failed to read hosts file \"%s\": %s\n",
            filename,
            strerror(errno)
        );
    hostname[hostname_idx] = '\0';
...
static bool handle_arg(
    ...
    switch (opt) {
        ...
        case OPT_MAX_RETRIES:
            nmap->max_retries = atou_check(value, 0, MAX_MAX_RETRIES, long_opt);
            break;
        case OPT_SCAN: parse_scan(value, &nmap->scans); break;
        case OPT_SPOOF_ADDRESS: parse_spoof_address(value, long_opt, nmap); break;
        case OPT_THREADS:
            nmap->num_threads = atou_check(value, 0, MAX_HOSTNAMES, long_opt);
            break;
        case OPT_TOP_PORTS:
            nmap->top_ports = MAX(
                nmap->top_ports,
                atou_check(value, 1, MAX_PORTS, long_opt)
            );
            break;
        case OPT_UDP_RATE:
            nmap->udp_sleep_us = 1000000 / atou_check(value, 1, 1000000, long_opt);
            break;
        default: break;
    }
    return true;
...
static bool
handle_long_opt(char* opt, size_t i, int* index, char** argv, t_nmap* nmap) {
...
static bool is_valid_opt(char** arg, int* index, t_nmap* nmap) {
    ...
    do
        for (size_t i = 0; valid_opt[i].opt; i++) {
            if (is_long_opt)
                if ((found_long_opt = handle_long_opt(*arg + 2, i, index, arg, nmap)) ==
                    true)
                    return true;
...
void parse_args(int argc, char* argv[], t_nmap* nmap) {
    ...
    if (nmap->scans == 0) {
        nmap->scans = nmap->is_sudo ? ~(1 << SCAN_CONN | 1 << SCAN_WIN)
                                    : (1 << SCAN_CONN);
    ...
    if (nmap->num_threads > nmap->hostname_count)
        nmap->num_threads = nmap->hostname_count;
