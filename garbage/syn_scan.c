...
int main(int argc, char* argv[]) {
    ...
    int sockfd =
        socket(AF_INET, SOCK_RAW, IPPROTO_TCP); // This is the main socket to send the SYN packet
    if (sockfd < 0)
        err_exit(
    ...
    for (host_count = 0; host_count < num_hosts; host_count++) {
        dest_ip.s_addr =
            inet_addr(dotted_quad(&target_in_addr)); // Current iteration's target host address
        if ((int)dest_ip.s_addr == -1) err_exit("Invalid address\n"); // weird cast
        ...
        if (pthread_create(&sniffer_thread, NULL, receive_ack, NULL) <
            0) // Thread to listen for just one SYN-ACK packet from any of the selected ports
            err_exit(
                "Could not create sniffer thread. Error number: %d. Error message: %s\n", errno,
                strerror(errno)
            );
        ...
        while (pch != NULL) { // Iterate all selected ports and send SYN packet all at once
            ...
            // fflush(stdout);
            if (sendto(
                    sockfd, datagram, sizeof(struct iphdr) + sizeof(struct tcphdr), 0,
        ...
        pthread_join(sniffer_thread, NULL); // Will wait for the sniffer to receive a reply, host is
                                            // considered closed if there aren't any
    ...
    printf("\nTotal active host: %d\n", total_open_host);
    printf(
        "Scan duration    : %d hour(s) %d min(s) %.05lf sec(s)\n", hours_duration, mins_duration,
        secs_duration
    );
 ...
 */
void prepare_datagram(
    char* datagram, const char* source_ip, struct iphdr* iph, struct tcphdr* tcph
) {
    ...
    tcph->check = 0; // If you set a checksum to zero, it's automatic
    tcph->urg_ptr = 0;
}
...
void parse_target(char* target, struct in_addr* target_in_addr, int64_t* num_hosts) {
    ...
    struct in_addr parsed_in_addr, mask_in_addr, wildcard_in_addr, network_in_addr,
        broadcast_in_addr, min_in_addr, max_in_addr;
...
void str_to_int(int* out, char* s, int base) {
    if (s[0] == '\0' || isspace((unsigned char)s[0])) return;

    char* end;
    errno = 0;
    long l = strtol(s, &end, base);

    if (l > INT_MAX || (errno == ERANGE && l == LONG_MAX)) return;
    if (l < INT_MIN || (errno == ERANGE && l == LONG_MIN)) return;
    if (*end != '\0') return;
...
char* hostname_to_ip(char* hostname) {
    ...
    if ((he = gethostbyname(hostname)) == NULL) err_exit("gethostbyname");

    addr_list = (struct in_addr**)he->h_addr_list;

    int a;
    for (a = 0; addr_list[a] != NULL; a++) return inet_ntoa(*addr_list[a]); // Return the first one;
...
void ip_to_host(const char* ip, char* buffer) {
    ...
    if (getnameinfo(
            (struct sockaddr*)&dest, sizeof(dest), buffer, NI_MAXHOST, NULL, 0, NI_NAMEREQD
        ) != 0)
